...
flowchart LR
  ...
  B -->|/v1/chat/completions, /v1/messages, /v1/models| C[Translators/Router]
...
flowchart LR
  ...
  B -->|Route aliases map to upstream /v1 handlers| C[Translators/Router]
  A -->|/api/auth, /api/user, /api/meta, /api/threads...| B
  B -->|Amp upstream proxy (config: amp-upstream-url)| F[(ampcode.com)]
